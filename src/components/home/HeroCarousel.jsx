...
// Novamente, tirei os slides estáticos de dentro do componente para ficar mais limpo
// e poupar processamento no React.
...
function HeroCarousel() {
  ...
  useEffect(() => {
    ...
    // Isto foi essencial: tive de adicionar este return.
    // Antes, se eu mudasse de página, o setInterval continuava a correr em background 
    // e criava um memory leak brutal. O return limpa o intervalo quando o componente desaparece.
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden group">
      ...
      <button onClick={prev} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-black/30 hover:bg-gym-yellow hover:text-black text-white rounded-full transition-all cursor-pointer opacity-0 group-hover:opacity-100 z-20">
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M15 19l-7-7 7-7" /></svg>
      </button>

      <button onClick={next} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-black/30 hover:bg-gym-yellow hover:text-black text-white rounded-full transition-all cursor-pointer opacity-0 group-hover:opacity-100 z-20">

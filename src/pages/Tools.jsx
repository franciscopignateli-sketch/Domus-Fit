...
// BASE DE DADOS LOCAL DE ALIMENTOS (Exemplo para a PAP)
const foodDatabase = [
...
function Tools() {
  const [activeTab, setActiveTab] = useState('imc'); // 'imc', 'calorias', ou 'macros'
  ...
  // Lógica Pesquisa de Alimentos (Autocomplete)
  ...
  const totalMacros = consumedFoods.reduce((acc, food) => {
    return {
        ...
        <div className="bg-gym-dark p-8 rounded-xl border border-white/10 shadow-2xl min-h-100">
          ...
          {activeTab === 'imc' && (
            <form onSubmit={calculateBMI} className="space-y-6 animate-fade-in">
              ...
              {bmiResult && (
                <div className="mt-6 p-6 bg-white/5 border border-gym-yellow rounded-lg text-center">
            ...
            <form onSubmit={calculateCalories} className="space-y-6 animate-fade-in">
              ...
              {calResult && (
                <div className="mt-6 p-6 bg-white/5 border border-gym-yellow rounded-lg text-center">
                  ...
                  <div className="flex justify-center gap-4 mt-4 text-xs uppercase font-bold">
                    <span className="bg-red-500/20 text-red-400 px-3 py-1 rounded">Perder Peso: {calResult - 300} kcal</span>
                    <span className="bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded">Ganhar Massa: {calResult + 300} kcal</span>
                  </div>
                </div>
              )}
            </form>
                ...
                {consumedFoods.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {consumedFoods.map((food, index) => (
                      <li key={index} className="flex justify-between items-center p-3 bg-white/5 rounded-lg border border-white/5">
                        <div>
              ...
              <div className="bg-black/40 p-4 rounded-xl border border-white/10">
                ...
                <div className="grid grid-cols-4 gap-2 text-center">
                  <div className="bg-gym-yellow/10 border border-gym-yellow/30 p-2 rounded">
                    <p className="text-xs text-gym-yellow uppercase font-bold">Calorias</p>
                    <p className="text-xl font-black text-white">{totalMacros.kcal.toFixed(0)}</p>
                  </div>
                  <div className="bg-blue-500/10 border border-blue-500/30 p-2 rounded">
                    <p className="text-xs text-blue-400 uppercase font-bold">Prot.</p>
                    <p className="text-xl font-black text-white">{totalMacros.prot.toFixed(1)}g</p>
                  </div>
                  <div className="bg-emerald-500/10 border border-emerald-500/30 p-2 rounded">
                    <p className="text-xs text-emerald-400 uppercase font-bold">Carbs</p>
                    <p className="text-xl font-black text-white">{totalMacros.carbs.toFixed(1)}g</p>
                  </div>
                  <div className="bg-red-500/10 border border-red-500/30 p-2 rounded">
                    <p className="text-xs text-red-400 uppercase font-bold">Gordura</p>
                    <p className="text-xl font-black text-white">{totalMacros.fat.toFixed(1)}g</p>
                  </div>
                </div>
              </div>
